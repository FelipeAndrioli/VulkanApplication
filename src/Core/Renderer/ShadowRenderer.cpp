...
void ShadowRenderer::Render(const VkCommandBuffer& commandBuffer, const std::vector<std::shared_ptr<Assets::Model>>& models, const glm::mat4& lightViewProj) {
	...
	gfxDevice->UpdateBuffer		(m_ShadowMappingUBO[gfxDevice->GetCurrentFrameIndex()], &m_ShadowMappingGPUData);
	...
	for (int i = 0; i < models.size(); i++) {
		...
		for (int j = 0; j < model->Meshes.size(); j++) {
			...
			vkCmdPushConstants	(commandBuffer, m_PSO.pipelineLayout, VK_SHADER_STAGE_ALL_GRAPHICS, 0, sizeof(PushConstant), &model->ModelIndex);
...
void ShadowRenderer::LoadResources() {
	m_RenderTarget = std::make_unique<Graphics::DepthOnlyRenderTarget>(m_Width, m_Height, m_Precision);
	...
	for (int i = 0; i < Graphics::FRAMES_IN_FLIGHT; i++) {
		...
		m_ShadowMappingUBO[i]	= gfxDevice->CreateBuffer(sizeof(ShadowMappingGPUData));

		gfxDevice->WriteSubBuffer(m_ModelUBO[i],			&m_ModelGPUData,			sizeof(ModelGPUData));
		gfxDevice->WriteSubBuffer(m_ShadowMappingUBO[i],	&m_ShadowMappingGPUData,	sizeof(ShadowMappingGPUData));
