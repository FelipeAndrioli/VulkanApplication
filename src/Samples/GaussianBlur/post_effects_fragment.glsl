...
layout (set = 1, binding = 0) uniform sampler2D bloom_pass_result;
...
void main() {
	...
	bool bloom_enabled				= bool(post_effects_gpu_data.flags & (1 << 4));

	vec3 hdr_color = texture(geometry_pass_result, uv).rgb;

	if (bloom_enabled) {
		vec3 bloom_color = texture(bloom_pass_result, uv).rgb;
